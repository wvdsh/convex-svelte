...
export class PaginatedQueryStateMachine<T> {
  ...
  onArgsChange(newArgsKey: string | null): {
    ...
    if (
      !this.state.haveArgsEverChanged &&
      this.state.initialArgsKey !== newArgsKey
    ) {
    ...
    const shouldKeep =
      this.config.keepPreviousData && this.state.results.length > 0;
  ...
  onError(error: Error): boolean {
    if (this.isInvalidCursorError(error)) {
      console.warn(
        "PaginatedQueryStateMachine: InvalidCursor detected, resetting:",
        error.message
      );
      this.state = {
        ...this.state,
  ...
  isUsingInitialData(): boolean {
    return (
      this.config.initialData !== undefined && !this.state.haveArgsEverChanged
    );
  }
